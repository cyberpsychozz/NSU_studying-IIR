...
int main(int argc, char *argv[]) {
    ...
    printf("Attempting mandatory lock on %s...\n", filename);
    printf("Note: Ensure 'chmod g+s %s' was run for mandatory locking to work.\n", filename);

    // Установка блокировки (mandatory, если файл имеет setgid-бит)
    if (fcntl(fd, F_SETLKW, &lock) == -1) {
        if (errno == EACCES || errno == EAGAIN) {
            fprintf(stderr, "File %s is already locked or access denied (mandatory lock active).\n", filename);
        } else {
            perror("fcntl mandatory lock failed");
        }
        close(fd);
        exit(EXIT_FAILURE);
    }

    printf("Mandatory lock acquired successfully. File is now locked.\n");
    printf("Launching editor (vi). Edit the file and save to test locking.\n");

    // Вызов редактора через system(3)
    char editor_cmd[256];
    snprintf(editor_cmd, sizeof(editor_cmd), "vi %s", filename);
